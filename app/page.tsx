...
const ReplyTextArea = memo(({ value, onChange, onSubmit }: {
  value: string;
...
function ConfessionsPage({ user }: ConfessionsPageProps) {
  ...
  const [replies, setReplies] = useState<{ $id: string; messageId: string; reply: string }[]>([]);
  ...
  useEffect(() => {
    const typed = new Typed(typedRef.current as Element, {
      strings: ["confess your love", "share your feelings", "confess your heart"],
      typeSpeed: 80,
      loop: true,
      showCursor: true,
      cursorChar: '|',
  ...
  const handleVote = async (postId: string, voteValue: number) => {
    ...
    if (voteData && voteData.userVote === voteValue) {
      return;
    }
    try {
      if (voteData && voteData.userVote !== null && voteData.docId) {
        ...
        await databases.createDocument(databaseId, votesCollectionId, "unique()", {
          postId,
          vote: voteValue,
          userId: user.$id,
        });
      ...
      <header className="relative w-full py-16 px-4 text-center">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="flex items-center justify-center">
            <p className="text-2xl font-bold text-pink-500 leading-none h-8" ref={typedRef}></p>
          </div>
        </div>
      </header>
